(function () {
  ...
  function render() {
    files = IgMD.uniqueFilenames(chats);
    ...
    chats.forEach(function (chat, i) {
      ...
      meta.className = "meta";
      meta.textContent = (chat.source || "Instagram") + " · " + turns + " messages · " + files[i];
  ...
  async function handleFile(file) {
    clearError();
    try {
      chats = await IgMD.parseFile(file);
      if (!chats.length) {
  ...
  listEl.addEventListener("click", async function (e) {
    ...
    var md = IgMD.toMarkdown(chats[i]);
    if (act === "copy") {
  ...
  allBtn.addEventListener("click", async function () {
    ...
    var zip = new JSZip();
    chats.forEach(function (chat, i) {
      zip.file(files[i], IgMD.toMarkdown(chat));
    });
    var blob = await zip.generateAsync({ type: "blob" });
    downloadBlob(blob, "igmd.zip");
  });
})();
