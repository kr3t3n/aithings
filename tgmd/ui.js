(function () {
  ...
  function render() {
    files = TgMD.uniqueFilenames(chats);
    ...
    chats.forEach(function (chat, i) {
      ...
      meta.className = "meta";
      meta.textContent = (chat.type || chat.source || "Telegram") + " · " + turns + " messages · " + files[i];
  ...
  async function handleFile(file) {
    clearError();
    try {
      chats = await TgMD.parseFile(file);
      if (!chats.length) {
        showError("No chats in this export.");
        return;
      }
      render();
    } catch (e) {
  ...
  listEl.addEventListener("click", async function (e) {
    ...
    var md = TgMD.toMarkdown(chats[i]);
    if (act === "copy") {
  ...
  allBtn.addEventListener("click", async function () {
    ...
    var zip = new JSZip();
    chats.forEach(function (chat, i) {
      zip.file(files[i], TgMD.toMarkdown(chat));
    });
    var blob = await zip.generateAsync({ type: "blob" });
    downloadBlob(blob, "tgmd.zip");
  });
})();
